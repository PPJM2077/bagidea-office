# ADR-0003: di module (foundation layer)
...
This ADR records the existence and position of the **`di`** module so the
`check-adr` gate has a documented decision for it. `di` lives in the
**foundation** layer, whose purpose is: cross-cutting primitives shared by every layer.

- Package: `@company-simulator/di`
...
Establish `di` as a module in the `foundation` layer with the dependencies
and health targets recorded above. The manifest at `foundation/di/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `di`'s layer, its public `exports`, or its upstream
...
*Derived from `foundation/di/module.json`. Expand Context/Decision as the module matures.*
