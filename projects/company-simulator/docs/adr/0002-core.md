# ADR-0002: core module (foundation layer)
...
This ADR records the existence and position of the **`core`** module so the
`check-adr` gate has a documented decision for it. `core` lives in the
**foundation** layer, whose purpose is: cross-cutting primitives shared by every layer.

- Package: `@company-simulator/core`
...
Establish `core` as a module in the `foundation` layer with the dependencies
and health targets recorded above. The manifest at `foundation/core/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `core`'s layer, its public `exports`, or its upstream
...
*Derived from `foundation/core/module.json`. Expand Context/Decision as the module matures.*
