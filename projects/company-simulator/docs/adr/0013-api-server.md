# ADR-0013: api-server module (apps layer)
...
This ADR records the existence and position of the **`api-server`** module so the
`check-adr` gate has a documented decision for it. `api-server` lives in the
**apps** layer, whose purpose is: delivery surfaces (CLI, API, UI) that wire features together.

- Package: `@company-simulator/api-server`
- Declared upstream dependencies:
- `@company-simulator/types` >=0.1.0
- `@company-simulator/domain` >=0.1.0
- `@company-simulator/analytics` >=0.1.0
- `@company-simulator/sim-run` >=0.1.0
- `@company-simulator/report-gen` >=0.1.0
- Health targets:
- maxPublicExports: 10
...
Establish `api-server` as a module in the `apps` layer with the dependencies
and health targets recorded above. The manifest at `apps/api-server/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `api-server`'s layer, its public `exports`, or its upstream
...
*Derived from `apps/api-server/module.json`. Expand Context/Decision as the module matures.*
