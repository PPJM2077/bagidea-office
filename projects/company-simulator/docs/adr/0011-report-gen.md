# ADR-0011: report-gen module (features layer)
...
This ADR records the existence and position of the **`report-gen`** module so the
`check-adr` gate has a documented decision for it. `report-gen` lives in the
**features** layer, whose purpose is: use-case orchestrations composed from core + ports.

- Package: `@company-simulator/report-gen`
...
Establish `report-gen` as a module in the `features` layer with the dependencies
and health targets recorded above. The manifest at `features/report-gen/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `report-gen`'s layer, its public `exports`, or its upstream
...
*Derived from `features/report-gen/module.json`. Expand Context/Decision as the module matures.*
