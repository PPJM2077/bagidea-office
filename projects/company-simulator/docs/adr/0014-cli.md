# ADR-0014: cli module (apps layer)
...
This ADR records the existence and position of the **`cli`** module so the
`check-adr` gate has a documented decision for it. `cli` lives in the
**apps** layer, whose purpose is: delivery surfaces (CLI, API, UI) that wire features together.

- Package: `@company-simulator/cli`
...
Establish `cli` as a module in the `apps` layer with the dependencies
and health targets recorded above. The manifest at `apps/cli/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `cli`'s layer, its public `exports`, or its upstream
...
*Derived from `apps/cli/module.json`. Expand Context/Decision as the module matures.*
