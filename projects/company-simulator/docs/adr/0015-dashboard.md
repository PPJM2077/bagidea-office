# ADR-0015: dashboard module (apps layer)
...
This ADR records the existence and position of the **`dashboard`** module so the
`check-adr` gate has a documented decision for it. `dashboard` lives in the
**apps** layer, whose purpose is: delivery surfaces (CLI, API, UI) that wire features together.

- Package: `@company-simulator/dashboard`
...
Establish `dashboard` as a module in the `apps` layer with the dependencies
and health targets recorded above. The manifest at `apps/dashboard/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `dashboard`'s layer, its public `exports`, or its upstream
...
*Derived from `apps/dashboard/module.json`. Expand Context/Decision as the module matures.*
