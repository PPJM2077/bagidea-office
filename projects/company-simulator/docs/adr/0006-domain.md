# ADR-0006: domain module (core layer)
...
This ADR records the existence and position of the **`domain`** module so the
`check-adr` gate has a documented decision for it. `domain` lives in the
**core** layer, whose purpose is: domain model and simulation engine -- the heart of the product.

- Package: `@company-simulator/domain`
- Declared upstream dependencies:
- `@company-simulator/types` >=0.1.0
- `@company-simulator/utils` >=0.1.0
- Health targets:
- maxPublicExports: 30
...
Establish `domain` as a module in the `core` layer with the dependencies
and health targets recorded above. The manifest at `core/domain/module.json`
is the source of truth for its public surface and dependency graph.

Any change to `domain`'s layer, its public `exports`, or its upstream
...
*Derived from `core/domain/module.json`. Expand Context/Decision as the module matures.*
