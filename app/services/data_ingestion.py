...
class _BaseClient(ABC):
    ...
    async def _request(
        ...
        *,
            ...
            try:
                ...
                if resp.status_code in {429, 503, 520, 529} and attempt < self._cfg.max_retries:
                    ...
                    log.warning(
                        ...
                        resp.status_code, url, attempt, self._cfg.max_retries, retry_after,
                    )
                ...
                last_exc = exc
                if attempt < self._cfg.max_retries and exc.response.status_code in {429, 503, 520, 529}:
        ...
        raise IOError(f"Request to {url} failed after {self._cfg.max_retries} attempts") from last_exc
...
class FREDClient(_BaseClient):
    ...
    async def fetch_series(
        ...
        *,
        ...
        body = await self._request("GET", f"{self._cfg.fred_rest_url}/series/observations", params=params)
...
class TwelveDataClient(_BaseClient):
    ...
    async def fetch_ohlcv(
        ...
        *interval* can be a ``TimeInterval`` enum or a raw string
        ...
        body = await self._request("GET", f"{self._cfg.twelvedata_rest_url}/time_series", params=params)
        ...
        *,
        ...
        return [
            OHLCV(
                ...
                interval=TimeInterval(_reverse_binance_interval(interval)) if isinstance(interval, str) else interval,
